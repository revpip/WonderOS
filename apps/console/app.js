...
function renderError(message) {
  workspace.innerHTML = `<div class="card error-card"><p class="eyebrow">WonderOS could not open this entity</p><h2>${escapeHtml(message)}</h2><p>Check the Wonder ID or confirm the API is running at ${escapeHtml(API_BASE)}.</p></div>`;
}
...
search.addEventListener('keydown', event => {
  ...
  if (!query) return;
  if (/^WND-[A-Z]{3}-\d{6}$/i.test(query)) return openEntity(query);
  const match = remembered.find(item => item.canonical_name.toLowerCase().includes(query.toLowerCase()));
  match ? renderEntity(match) : renderError(`No remembered entity matches “${query}”. Search by Wonder ID to query the canonical API directly.`);
});
